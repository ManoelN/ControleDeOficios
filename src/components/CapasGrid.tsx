...
import { Capa } from '../lib/supabase';
import { useState } from 'react';
import { ConfirmModal } from './ConfirmModal';

interface CapasGridProps {
  capas: Capa[];
  onMarcarCapa: (numero: number, status: 'disponivel' | 'utilizado', descricao?: string) => Promise<void>;
  onMarcarProximo: (descricao?: string) => Promise<number | null>;
  loading: boolean;
}

export function CapasGrid({ capas, onMarcarCapa, onMarcarProximo, loading }: CapasGridProps) {
  ...
  const handleClick = (capa: Capa) => {
    if (processingNumero) return;

    const acao = capa.status === 'disponivel' ? 'marcar' : 'desmarcar';
    setConfirmAction({ numero: capa.numero, acao });
  };

  const handleConfirm = async (descricao: string) => {
    if (!confirmAction) return;

    const capa = capas.find((c) => c.numero === confirmAction.numero);
    if (!capa) return;

    const novoStatus = capa.status === 'disponivel' ? 'utilizado' : 'disponivel';

    setConfirmAction(null);
    setProcessingNumero(confirmAction.numero);

    try {
      await onMarcarCapa(confirmAction.numero, novoStatus, descricao);
    } catch (error) {
  ...
  const handleMarcarProximo = async () => {
    ...
    try {
      ...
      if (numero) {
        const element = document.getElementById(`capa-${numero}`);
        if (element) {
  ...
  const filteredCapas = capas.filter((c) => {
    if (searchTerm) {
      const matchesNumero = c.numero.toString().includes(searchTerm);
      const matchesDescricao = c.descricao?.toLowerCase().includes(searchTerm.toLowerCase());
      if (!matchesNumero && !matchesDescricao) return false;
    }
    if (showOnlyAvailable && c.status !== 'disponivel') return false;
    return true;
  });

  const stats = {
    total: capas.length,
    utilizados: capas.filter((c) => c.status === 'utilizado').length,
    disponiveis: capas.filter((c) => c.status === 'disponivel').length,
  };
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="mb-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
              <div className="text-sm text-blue-600 font-medium mb-1">Total de Capas</div>
              <div className="text-3xl font-bold text-blue-700">{stats.total}</div>
            </div>
            <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-4 border border-yellow-200">
              <div className="text-sm text-yellow-600 font-medium mb-1">Utilizadas</div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="showOnlyAvailable" className="text-sm text-gray-700 cursor-pointer">
                Mostrar apenas capas disponíveis
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="mb-4 flex items-center gap-6 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-600">Utilizada</span>
          </div>
        </div>
        ...
        <div className="border rounded-lg overflow-hidden">
          <div className="grid grid-cols-5 sm:grid-cols-10 md:grid-cols-10 gap-0">
            {filteredCapas.map((capa) => {
              const isProcessing = processingNumero === capa.numero;
              ...
              return (
                ...
                >
                  <span className="relative z-10 font-bold">
                    {capa.numero.toString().padStart(3, '0')}
                  </span>
                  {capa.descricao && capa.status === 'utilizado' && (
        ...
        {filteredCapas.length === 0 && searchTerm && (
          <div className="text-center py-8 text-gray-500">
            <p>Nenhuma capa encontrada com o número "{searchTerm}"</p>
          </div>
        )}
      </div>
