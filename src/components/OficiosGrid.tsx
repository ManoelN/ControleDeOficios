...
import { Oficio } from '../lib/supabase';
import { useState } from 'react';
import { ConfirmModal } from './ConfirmModal';

interface OficiosGridProps {
  oficios: Oficio[];
...
export function OficiosGrid({ oficios, onMarcarOficio, onMarcarProximo, loading }: OficiosGridProps) {
  ...
  const handleClick = (oficio: Oficio) => {
  ...
  const handleMarcarProximo = async () => {
    ...
    try {
      ...
      if (numero) {
        const element = document.getElementById(`oficio-${numero}`);
        if (element) {
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-12 text-center">
        ...
        <p className="mt-4 text-gray-600">Carregando ofícios...</p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="mb-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
              <div className="text-sm text-blue-600 font-medium mb-1">Total de Ofícios</div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="showOnlyAvailable" className="text-sm text-gray-700 cursor-pointer">
                Mostrar apenas ofícios disponíveis
              </label>
            </div>
          </div>
        </div>
        ...
        {filteredOficios.length === 0 && searchTerm && (
          <div className="text-center py-8 text-gray-500">
            <p>Nenhum ofício encontrado com o número "{searchTerm}"</p>
          </div>
        )}
      </div>
