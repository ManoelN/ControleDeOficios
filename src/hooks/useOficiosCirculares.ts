...
import { supabase, OficioCircular } from '../lib/supabase';
...
export function useOficiosCirculares(anoId: string | null) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('oficios_circulares_changes')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'oficios_circulares',
          filter: `ano_id=eq.${anoId}`,
        },
        (payload) => {
          if (payload.eventType === 'INSERT') {
            setOficios((prev) => [...prev, payload.new as OficioCircular]);
          } else if (payload.eventType === 'UPDATE') {
            setOficios((prev) =>
              prev.map((o) => (o.id === payload.new.id ? (payload.new as OficioCircular) : o))
            );
  ...
  async function loadOficios() {
    ...
    while (hasMore) {
      ...
      if (error) {
        console.error('Erro ao carregar ofícios circulares:', error);
        break;
      }
